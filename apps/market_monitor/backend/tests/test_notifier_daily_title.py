...
class DailyPushTitleTests(unittest.TestCase):
    def test_first_push_adds_warning_after_daily_count(self) -> None:
        message = "**🚀 启动预警 | SOXS（触发1次）**\n正文"

        updated = AlertNotifier._apply_daily_push_title(message, 1)

        self.assertEqual(updated, "**🚀 启动预警 | SOXS（今日第1次推送）⚠️**\n正文")
    ...
    def test_reformatting_does_not_duplicate_or_retain_warning(self) -> None:
        first = "**🚀 启动预警 | SOXS（今日第1次推送）⚠️**\n正文"

        repeated_first = AlertNotifier._apply_daily_push_title(first, 1)
        second = AlertNotifier._apply_daily_push_title(first, 2)

        self.assertEqual(repeated_first.count("⚠️"), 1)
